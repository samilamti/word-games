...
- `scripts/audio/` — background music loop via Stable Audio Open (**gated model**: needs a one-time licence acceptance + `HF_TOKEN`). `make_loop.sh` trims, crossfades the seam and encodes AAC to `public/audio/music-main.m4a`. Sound effects stay procedural in `SoundManager`
