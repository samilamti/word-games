...
- `scripts/art/` — local SDXL character art. Combat sprites are img2img paint-overs of the Blender renders (preserves silhouette, framing and the feet-at-95% anchor `BattleOverlay` depends on; alpha is re-applied from the master, so no background remover). Portraits are txt2img for the arrival toast and victory/defeat cards. See its README for the workflow and the traps
- `scripts/audio/` — background music loop via Stable Audio Open (**gated model**: needs a one-time licence acceptance + `HF_TOKEN`). `make_loop.sh` trims, crossfades the seam, limits to -1 dB and encodes AAC to `public/audio/music-main.m4a` (shipped: 36 s, 570 KB, from seed 3 — the flattest of six candidates, which is what a bed looping behind a word game wants). Sound effects stay procedural in `SoundManager`. A missing loop is not an error: `MusicBed` logs one warning and the game runs silent
...
- `lexica_knights_settings` — accessibility/feel toggles (`reduceMotion`, `soundEnabled`, `musicEnabled`, `hapticsEnabled`) + `tutorialSeen`; `settingsStore`, defaults `reduceMotion` from `prefers-reduced-motion`. To re-test the first-run tutorial, flip `tutorialSeen` to false in this JSON (or clear the key)

## Game design

- 13×13 board with premium squares (letter ×2/×3, word ×2/×3, GEM_FORGE, VOID, CENTER ★). Squares are marked with a `×N` glyph, **not** the old DL/TL/DW/TW codes — beta testers couldn't decode them, and at a 20px mobile tile the labels rendered at 5.6px. Letter-vs-word scope is carried by the colour family (cool = letter, warm = word), an underline bar under word multipliers, the `PremiumHintToast` that names the square as a tile lands on it, and `BoardLegend`. All of it renders from `components/PremiumMarker.tsx`, which is the single source of truth for premium colours and glyphs
- Turn-based combat: spell word → deal damage → enemy counterattacks → refill tiles → repeat
- Damage = `SUM(tile values) × premium_word_multiplier × word_length_multiplier × COMBAT_SCALAR(3)`
- 5-enemy campaign. Difficulty was rebalanced 2026-08-14 after beta feedback that the opening demanded long words; `src/types/enemies.test.ts` documents and enforces the curve. Three levers per enemy: `maxWordLen`/`maxNewTiles` bound the NPC's raw score, `damageMultiplier` shapes ordinary hits, `maxDamagePerTurn` caps the spikes. HP 60 → 210, incoming ≈ 6 → 19 per turn (linear). **`pickPercentile` replaced `pickRank`**: an absolute rank silently inverted on sparse boards, where a rank of 4 clamps to "play the best move" — i.e. against a beginner on turn one. `attack`/`defense` on `EnemyDef` are cosmetic HP-bar text; `attack` is kept equal to `maxDamagePerTurn` so the displayed number is honest
- First-run guide: `components/TutorialModal.tsx`, 5 steps, opens itself during the dictionary load when `!tutorialSeen`. Diagrams are built from the game's own tile/cell CSS (no image assets, self-localizing). Skip is on every step and every exit marks it seen. Replay from Settings → How to play, or the `?` button. Closing the auto-run replays the enemy-spawn toast (`gameStore.replayEnemySpawnToast`), which would otherwise expire behind the modal
